...
using DataTableReader = Tarantino.Commons.Core.Services.DataFileManagement.Impl.DataTableReader;

namespace Tarantino.UnitTests.Commons.Core.Services.DataFileManagement
{
	[TestFixture]
	public class DataTableReaderTester
	{
		...
		[Test, ExpectedException(ExceptionType = typeof(ApplicationException), ExpectedMessage = "'Up' is not a valid value in Tarantino.Commons.Core.Model.Enumerations.PersistenceMode")]
		public void Handles_scenario_When_Invalid_enumerated_value_read()
		{
			DataTable table = new DataTable();
			table.Columns.Add("Mode Column");
			DataRow row1 = table.NewRow();
			...
			Assert.That(tableReader.Read(), Is.EqualTo(true));
			tableReader.GetEnumeration<PersistenceMode>("Mode Column");
		}
	}
}